...
(defn read-from-pubsub
  ...
  (let [pipe (if (instance? Pipeline p) (PBegin/in p) p)]
    (cond
      (= :subscription kind) (apply-transform pipe (.subscription PubsubIO$Read subscription-or-topic) {} options)
      (= :topic kind) (apply-transform pipe (.topic PubsubIO$Read subscription-or-topic) {} options)
...
(defn write-to-pubsub
  ...
  [topic options pcoll]
  (-> pcoll
      (apply-transform (topic PubsubIO$Write topic) {} options)))
